// contracts: docs/contracts/ux-flows.md (공통 상태머신, 쇼/투표 흐름)
...
function StateSkeleton({
  ...
  if (state === "loading") return <Text>{title} 데이터를 불러오는 중...</Text>;
  if (state === "empty") return <Text>{title} 데이터가 아직 없어요.</Text>;
...
export default function HomeTabScreen() {
  ...
  return (
    <View style={{ flex: 1, padding: 16, gap: 12 }}>
      ...
      <StateSkeleton title="홈" state={state} />
      <View style={{ gap: 8 }}>
